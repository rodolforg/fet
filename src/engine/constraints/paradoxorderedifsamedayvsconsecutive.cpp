...
#include "paradoxorderedifsamedayvsconsecutive.h"
...
bool ParadoxOrderedIfSameDayVsConsecutive::prepare(const Rules& rules)
{
	bool ok = true;
			...
			if (constrTwoActivitiesOrderedIfSameDayPercentages[ai1][i] >= 100) {
				...
				for (int mai2 = 0; mai2 < constrTwoActivitiesConsecutiveActivities[ai2].size(); mai2++) {
					if (constrTwoActivitiesConsecutiveActivities[ai2][mai2] == ai1) {
						...
						s+="\n";
						s+=GeneratePreTranslate::tr("They should be consecutive activities, but there is a constraint Two Ordered Activities If On Same Day that demands a reversed activity order.");
						s+=QString("\n----\n%1\n----\n%2")
