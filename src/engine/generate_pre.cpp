...
MinContinuousGapInIntervalForStudents minContinuousGapInIntervalForStudentsList;
...
{
	for (const ErrorCode& error : qAsConst(errorList)) {
...
bool processTimeSpaceConstraints(QWidget* parent, QTextStream* initialOrderStream)
{
	...
	t=minContinuousGapInIntervalForStudentsList.prepare(gt.rules);
	if(!t) {
		reportSkippableErrors(parent, minContinuousGapInIntervalForStudentsList.getErrors());
		return false;
	}
	...
	ParadoxMinGapsVsMinDaysConsecutive paradoxMinGapsMinDaysConsecutive;
	ParadoxMinGapsVsConsecutive paradoxMinGapsConsecutive;
	ParadoxMinGapsVs2Grouped paradoxMinGaps2Grouped;
	...
	for (ConstraintPre* paradox : qAsConst(paradoxes)) {
