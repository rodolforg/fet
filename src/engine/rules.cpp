	...
	if(initialStudentsSetName!=finalStudentsSetName){
		foreach(TimeConstraint* ctr, timeConstraintsList){
			...
			//2017-02-07
				...
				ConstraintStudentsSetMinContinuousGapInInterval* crt_constraint=(ConstraintStudentsSetMinContinuousGapInInterval*)ctr;
				if(initialStudentsSetName == crt_constraint->students)
					crt_constraint->students=finalStudentsSetName;
			}
		}
...
void Rules::updateConstraintsAfterRemoval()
{
	...
	foreach(TimeConstraint* tc, timeConstraintsList){
		...
		else if(tc->type==CONSTRAINT_STUDENTSSET_MIN_CONTINUOUS_GAP_IN_INTERVAL){
			ConstraintStudentsSetMinContinuousGapInInterval* c=(ConstraintStudentsSetMinContinuousGapInInterval*)tc;
			if(!permanentStudentsHash.contains(c->students))
				toBeRemovedTime.append(tc);
		}
	}
...
bool Rules::read(QWidget* parent, const QString& fileName, const QString& outputDirPath)
{
	...
	QString sTime = QDateTime::currentDateTime().toString(Qt::DefaultLocaleShortDate);

	log.minimum("Log generated by FET "+FET_VERSION+" on "+sTime+"\n\n");
	QFileInfo fileinfo(fileName);
	QString shortFileName=fileinfo.fileName();
	log.minimum("Reading file "+shortFileName+"\n");
	log.minimum("Complete file name, including path: "+QDir::toNativeSeparators(fileinfo.absoluteFilePath())+"\n");
	log.minimum("\n");

	QString logFilePath = logPathString + FILE_SEP + XML_PARSING_LOG_FILENAME;
...
/*			QDomAttr a=elem1.attributeNode("version");
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Institution_Name"){
			...
			log.minimum("Read institution name="+this->institutionName+"\n");
		}
		else if(xmlReader.name()=="Comments"){
			QString text=xmlReader.readElementText();
			this->comments=text;
			log.minimum("Read comments="+this->comments+"\n");
		}
			...
			while(xmlReader.readNextStartElement()){
				...
				//old .fet XML format
					...
					if(tmp>=MAX_DAYS_PER_WEEK){
						...
						log.verbose("   Found day "+this->daysOfTheWeek[tmp]+"\n");
						tmp++;
					}
				}
			...
			while(xmlReader.readNextStartElement()){
				...
				if(xmlReader.name()=="Number" || xmlReader.name()=="Number_of_Hours"){
					...
					numberWasFound=true;
					log.verbose("   Found the number of hours per day = "+
				...
				//old .fet XML format
					...
					if(tmp>=MAX_HOURS_PER_DAY){
						...
						log.verbose("   Found hour "+this->hoursOfTheDay[tmp]+"\n");
						tmp++;
					}
				}
				//end old .fet XML format
				...
				if(xmlReader.name()=="Teacher"){
					...
					while(xmlReader.readNextStartElement()){
						...
						if(xmlReader.name()=="Name"){
							QString text=xmlReader.readElementText();
							teacher->name=text;
							log.verbose("    Read teacher name: "+teacher->name+"\n");
						}
						else if(xmlReader.name()=="Target_Number_of_Hours"){
							QString text=xmlReader.readElementText();
							teacher->targetNumberOfHours=text.toInt();
							log.verbose("    Read teacher target number of hours: "+CustomFETString::number(teacher->targetNumberOfHours)+"\n");
						}
							...
							log.verbose("    Crt. teacher comments="+teacher->comments+"\n");
						}
						else{
							xmlReader.skipCurrentElement();
							log.numberOfUnrecognizedFields++;
			...
			if(!(tmp==this->teachersList.size()))
				...
				log.minimum("Added "+CustomFETString::number(tmp)+" teachers\n");
			}
		}
				...
				if(xmlReader.name()=="Subject"){
					...
					while(xmlReader.readNextStartElement()){
						...
						if(xmlReader.name()=="Name"){
							QString text=xmlReader.readElementText();
							subject->name=text;
							log.verbose("    Read subject name: "+subject->name+"\n");
						}
						else if(xmlReader.name()=="Comments"){
							QString text=xmlReader.readElementText();
							subject->comments=text;
							log.verbose("    Crt. subject comments="+subject->comments+"\n");
						}
						else{
							xmlReader.skipCurrentElement();
							log.numberOfUnrecognizedFields++;
			...
			if(!(tmp==this->subjectsList.size()))
				...
				log.minimum("Added "+CustomFETString::number(tmp)+" subjects\n");
			}
		}
				...
				if(xmlReader.name()=="Subject_Tag"){
					...
					while(xmlReader.readNextStartElement()){
						...
						if(xmlReader.name()=="Name"){
							...
							log.verbose("    Crt. activity tag comments="+activityTag->comments+"\n");
						}
						else{
							xmlReader.skipCurrentElement();
							log.numberOfUnrecognizedFields++;
					...
					if(tmp2){
						...
						tmp++;
						log.verbose("   Activity tag inserted\n");
					}
				}
				else{
					xmlReader.skipCurrentElement();
			...
			if(!(tmp==this->activityTagsList.size()))
				...
				log.minimum("Added "+CustomFETString::number(tmp)+" activity tags\n");
			}
		}
				...
				if(xmlReader.name()=="Year"){
					...
					while(xmlReader.readNextStartElement()){
						...
						if(xmlReader.name()=="Name"){
							...
							log.verbose("    Read year name: "+sty->name+"\n");
						}
						else if(xmlReader.name()=="Number_of_Students"){
							QString text=xmlReader.readElementText();
							sty->numberOfStudents=text.toInt();
							log.verbose("    Read year number of students: "+CustomFETString::number(sty->numberOfStudents)+"\n");
						}
						else if(xmlReader.name()=="Comments"){
							QString text=xmlReader.readElementText();
							sty->comments=text;
							log.verbose("    Crt. year comments="+sty->comments+"\n");
						}
							...
							while(xmlReader.readNextStartElement()){
								...
								if(xmlReader.name()=="Name"){
									...
									log.verbose("     Read group name: "+stg->name+"\n");
								}
								else if(xmlReader.name()=="Number_of_Students"){
									QString text=xmlReader.readElementText();
									stg->numberOfStudents=text.toInt();
									log.verbose("     Read group number of students: "+CustomFETString::number(stg->numberOfStudents)+"\n");
								}
								else if(xmlReader.name()=="Comments"){
									QString text=xmlReader.readElementText();
									stg->comments=text;
									log.verbose("    Crt. group comments="+stg->comments+"\n");
								}
									...
									while(xmlReader.readNextStartElement()){
										...
										if(xmlReader.name()=="Name"){
											...
											log.verbose("     Read subgroup name: "+sts->name+"\n");
										}
										else if(xmlReader.name()=="Number_of_Students"){
											QString text=xmlReader.readElementText();
											sts->numberOfStudents=text.toInt();
											log.verbose("     Read subgroup number of students: "+CustomFETString::number(sts->numberOfStudents)+"\n");
										}
										else if(xmlReader.name()=="Comments"){
											QString text=xmlReader.readElementText();
											sts->comments=text;
											log.verbose("    Crt. subgroup comments="+sts->comments+"\n");
										}
										else{
											xmlReader.skipCurrentElement();
											log.numberOfUnrecognizedFields++;
			...
			log.minimum("Added "+CustomFETString::number(ny)+" students years\n");
			log.minimum("Added "+CustomFETString::number(tgr)+" students groups\n");
			log.minimum("Added "+CustomFETString::number(tsgr)+" students subgroups\n");
				...
				if(xmlReader.name()=="Activity"){
					...
					while(xmlReader.readNextStartElement()){
						...
						if(xmlReader.name()=="Weekly"){
							xmlReader.skipCurrentElement();
							log.verbose("    Current activity is weekly - ignoring tag\n");
						...
						//old tag
							...
							xmlReader.skipCurrentElement();
							log.verbose("    Current activity is fortnightly - ignoring tag\n");
							//assert(p==PARITY_NOT_INITIALIZED);
							...
							if(text=="yes" || text=="true" || text=="1"){
								ac=true;
								log.verbose("	Current activity is active\n");
							}
							else{
								...
								ac=false;
								log.verbose("	Current activity is not active\n");
							}
						}
						else if(xmlReader.name()=="Comments"){
							QString text=xmlReader.readElementText();
							cm=text;
							log.verbose("    Crt. activity comments="+cm+"\n");
						}
						else if(xmlReader.name()=="Teacher"){
							QString text=xmlReader.readElementText();
							tn=text;
							log.verbose("    Crt. activity teacher="+tn+"\n");
							...
							QString text=xmlReader.readElementText();
							sjn=text;
							log.verbose("    Crt. activity subject="+sjn+"\n");
							...
							QString text=xmlReader.readElementText();
							atn=text;
							log.verbose("    Crt. activity activity_tag="+atn+"\n");
							...
							QString text=xmlReader.readElementText();
							stn=text;
							log.verbose("    Crt. activity students+="+stn+"\n");
							...
							QString text=xmlReader.readElementText();
							d=text.toInt();
							log.verbose("    Crt. activity duration="+CustomFETString::number(d)+"\n");
						}
						else if(xmlReader.name()=="Total_Duration"){
							QString text=xmlReader.readElementText();
							td=text.toInt();
							log.verbose("    Crt. activity total duration="+CustomFETString::number(td)+"\n");
						}
						else if(xmlReader.name()=="Id"){
							QString text=xmlReader.readElementText();
							id=text.toInt();
							log.verbose("    Crt. activity id="+CustomFETString::number(id)+"\n");
						}
						else if(xmlReader.name()=="Activity_Group_Id"){
							QString text=xmlReader.readElementText();
							gid=text.toInt();
							log.verbose("    Crt. activity group id="+CustomFETString::number(gid)+"\n");
						}
						else if(xmlReader.name()=="Number_Of_Students"){
							QString text=xmlReader.readElementText();
							cnos=false;
							nos=text.toInt();
							log.verbose("    Crt. activity number of students="+CustomFETString::number(nos)+"\n");
						}
						else{
							xmlReader.skipCurrentElement();
							log.numberOfUnrecognizedFields++;
					...
					if(id<0)
						...
						na++;
						log.verbose("   Added the activity\n");
					}
					else{
						xmlReader.raiseError(tr("The activity with id=%1 contains incorrect data").arg(id));
						/*log.verbose("   Activity with id ="+CustomFETString::number(id)+" contains invalid data - skipping\n");
						RulesReconcilableMessage::warning(parent, tr("FET information"),
				...
				if(xmlReader.name()=="Building"){
					...
					while(xmlReader.readNextStartElement()){
						...
						if(xmlReader.name()=="Name"){
							QString text=xmlReader.readElementText();
							bu->name=text;
							log.verbose("    Read building name: "+bu->name+"\n");
						}
						else if(xmlReader.name()=="Comments"){
							QString text=xmlReader.readElementText();
							bu->comments=text;
							log.verbose("    Crt. building comments="+bu->comments+"\n");
						}
						else{
							xmlReader.skipCurrentElement();
							log.numberOfUnrecognizedFields++;
			...
			if(!(tmp==this->buildingsList.size()))
				...
				log.minimum("Added "+CustomFETString::number(tmp)+" buildings\n");
			}
		}
				...
				if(xmlReader.name()=="Room"){
					...
					while(xmlReader.readNextStartElement()){
						...
						if(xmlReader.name()=="Name"){
							QString text=xmlReader.readElementText();
							rm->name=text;
							log.verbose("    Read room name: "+rm->name+"\n");
						}
						else if(xmlReader.name()=="Type"){
							//rm->type=text;
							xmlReader.skipCurrentElement();
							log.verbose("    Ignoring old tag room type:\n");
						}
						else if(xmlReader.name()=="Capacity"){
							QString text=xmlReader.readElementText();
							rm->capacity=text.toInt();
							log.verbose("    Read room capacity: "+CustomFETString::number(rm->capacity)+"\n");
						}
						else if(xmlReader.name()=="Equipment"){
							//rm->addEquipment(text);
							xmlReader.skipCurrentElement();
							log.verbose("    Ignoring old tag - room equipment:\n");
						}
						else if(xmlReader.name()=="Building"){
							QString text=xmlReader.readElementText();
							rm->building=text;
							log.verbose("    Read room building:\n"+rm->building);
						}
						else if(xmlReader.name()=="Comments"){
							QString text=xmlReader.readElementText();
							rm->comments=text;
							log.verbose("    Crt. room comments="+rm->comments+"\n");
						}
						else{
							xmlReader.skipCurrentElement();
							log.numberOfUnrecognizedFields++;
			...
			if(!(tmp==this->roomsList.size()))
				...
				log.minimum("Added "+CustomFETString::number(tmp)+" rooms\n");
			}
		}
			...
			while(xmlReader.readNextStartElement()){
				...
				if(xmlReader.name()=="ConstraintBasicCompulsoryTime"){
					...
					crt_constraint=readTeacherNotAvailableTimes(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintTeacherMaxDaysPerWeek"){
					crt_constraint=readTeacherMaxDaysPerWeek(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintTeachersMaxDaysPerWeek"){
					crt_constraint=readTeachersMaxDaysPerWeek(xmlReader, log);
				}

				else if(xmlReader.name()=="ConstraintTeacherMinDaysPerWeek"){
					crt_constraint=readTeacherMinDaysPerWeek(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintTeachersMinDaysPerWeek"){
					crt_constraint=readTeachersMinDaysPerWeek(xmlReader, log);
				}
					...
					crt_constraint=readMinGapsBetweenActivities(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintActivitiesNotOverlapping"){
					crt_constraint=readActivitiesNotOverlapping(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintActivitiesSameStartingTime"){
					crt_constraint=readActivitiesSameStartingTime(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintActivitiesSameStartingHour"){
					crt_constraint=readActivitiesSameStartingHour(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintActivitiesSameStartingDay"){
					crt_constraint=readActivitiesSameStartingDay(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintTeachersMaxHoursDaily"){
					crt_constraint=readTeachersMaxHoursDaily(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintTeacherMaxHoursDaily"){
					crt_constraint=readTeacherMaxHoursDaily(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintTeachersMaxHoursContinuously"){
					crt_constraint=readTeachersMaxHoursContinuously(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintTeacherMaxHoursContinuously"){
					crt_constraint=readTeacherMaxHoursContinuously(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintTeacherActivityTagMaxHoursContinuously"){
					crt_constraint=readTeacherActivityTagMaxHoursContinuously(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintTeachersActivityTagMaxHoursContinuously"){
					crt_constraint=readTeachersActivityTagMaxHoursContinuously(xmlReader, log);
				}

				else if(xmlReader.name()=="ConstraintTeacherActivityTagMaxHoursDaily"){
					crt_constraint=readTeacherActivityTagMaxHoursDaily(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintTeachersActivityTagMaxHoursDaily"){
					crt_constraint=readTeachersActivityTagMaxHoursDaily(xmlReader, log);
				}
				 ...
				 || xmlReader.name()=="ConstraintTeachersSubgroupsNoMoreThanXHoursDaily") && !skipDeprecatedConstraints){
					...
					crt_constraint=readStudentsMaxHoursDaily(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintStudentsSetMaxHoursDaily"){
					crt_constraint=readStudentsSetMaxHoursDaily(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintStudentsMaxHoursContinuously"){
					crt_constraint=readStudentsMaxHoursContinuously(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintStudentsSetMaxHoursContinuously"){
					crt_constraint=readStudentsSetMaxHoursContinuously(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintStudentsSetActivityTagMaxHoursContinuously"){
					crt_constraint=readStudentsSetActivityTagMaxHoursContinuously(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintStudentsActivityTagMaxHoursContinuously"){
					crt_constraint=readStudentsActivityTagMaxHoursContinuously(xmlReader, log);
				}

				else if(xmlReader.name()=="ConstraintStudentsSetActivityTagMaxHoursDaily"){
					crt_constraint=readStudentsSetActivityTagMaxHoursDaily(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintStudentsActivityTagMaxHoursDaily"){
					crt_constraint=readStudentsActivityTagMaxHoursDaily(xmlReader, log);
				}
					...
					crt_constraint=readActivitiesEndStudentsDay(xmlReader, log);
				}
				//old, with 2 and 3
				else if(xmlReader.name()=="Constraint2ActivitiesConsecutive"){
					crt_constraint=read2ActivitiesConsecutive(xmlReader, log);
				}
				else if(xmlReader.name()=="Constraint2ActivitiesGrouped"){
					crt_constraint=read2ActivitiesGrouped(xmlReader, log);
				}
				else if(xmlReader.name()=="Constraint3ActivitiesGrouped"){
					crt_constraint=read3ActivitiesGrouped(xmlReader, log);
				}
				else if(xmlReader.name()=="Constraint2ActivitiesOrdered"){
					crt_constraint=read2ActivitiesOrdered(xmlReader, log);
				}
				//end old
				else if(xmlReader.name()=="ConstraintTwoActivitiesConsecutive"){
					crt_constraint=readTwoActivitiesConsecutive(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintTwoActivitiesGrouped"){
					crt_constraint=readTwoActivitiesGrouped(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintThreeActivitiesGrouped"){
					crt_constraint=readThreeActivitiesGrouped(xmlReader, log);
				}
					...
					crt_constraint=readActivityPreferredTimeSlots(xmlReader, log);
				}
					...
					crt_constraint=readBreakTimes(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintTeachersNoGaps"){
					crt_constraint=readTeachersNoGaps(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintTeachersMaxGapsPerWeek"){
					crt_constraint=readTeachersMaxGapsPerWeek(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintTeacherMaxGapsPerWeek"){
					crt_constraint=readTeacherMaxGapsPerWeek(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintTeachersMaxGapsPerDay"){
					crt_constraint=readTeachersMaxGapsPerDay(xmlReader, log);
				}
					...
					crt_constraint=readStudentsMaxGapsPerWeek(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintStudentsSetMaxGapsPerWeek"){
					crt_constraint=readStudentsSetMaxGapsPerWeek(xmlReader, log);
				}

				else if(xmlReader.name()=="ConstraintStudentsMaxGapsPerDay"){
					crt_constraint=readStudentsMaxGapsPerDay(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintStudentsSetMaxGapsPerDay"){
					crt_constraint=readStudentsSetMaxGapsPerDay(xmlReader, log);
				}
					...
					crt_constraint=readActivitiesPreferredTimeSlots(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintActivitiesPreferredStartingTimes"){
					crt_constraint=readActivitiesPreferredStartingTimes(xmlReader, log);
				}
////////////////
				else if(xmlReader.name()=="ConstraintSubactivitiesPreferredTimeSlots"){
					crt_constraint=readSubactivitiesPreferredTimeSlots(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintSubactivitiesPreferredStartingTimes"){
					crt_constraint=readSubactivitiesPreferredStartingTimes(xmlReader, log);
				}
////////////////2011-09-25
				else if(xmlReader.name()=="ConstraintActivitiesOccupyMaxTimeSlotsFromSelection"){
					crt_constraint=readActivitiesOccupyMaxTimeSlotsFromSelection(xmlReader, log);
				}
////////////////
////////////////2011-09-30
				else if(xmlReader.name()=="ConstraintActivitiesMaxSimultaneousInSelectedTimeSlots"){
					crt_constraint=readActivitiesMaxSimultaneousInSelectedTimeSlots(xmlReader, log);
				}
////////////////
				...
				/////////begin 2017-02-07
					...
					crt_constraint=readTeacherMaxSpanPerDay(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintTeachersMaxSpanPerDay"){
					crt_constraint=readTeachersMaxSpanPerDay(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintStudentsSetMaxSpanPerDay"){
					crt_constraint=readStudentsSetMaxSpanPerDay(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintStudentsMaxSpanPerDay"){
					crt_constraint=readStudentsMaxSpanPerDay(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintTeacherMinRestingHours"){
					crt_constraint=readTeacherMinRestingHours(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintTeachersMinRestingHours"){
					crt_constraint=readTeachersMinRestingHours(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintStudentsSetMinRestingHours"){
					crt_constraint=readStudentsSetMinRestingHours(xmlReader, log);
				}
				...
				else if(xmlReader.name()=="ConstraintStudentsSetMinContinuousGapInInterval"){
					crt_constraint=readStudentsSetMinContinuousGapInInterval(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintStudentsMinContinuousGapInInterval"){
					crt_constraint=readStudentsMinContinuousGapInInterval(xmlReader, log);
				}
				/////////
				else{
					xmlReader.skipCurrentElement();
...
//corruptConstraintTime:
			...
			while(xmlReader.readNextStartElement()){
				...
				if(xmlReader.name()=="ConstraintBasicCompulsorySpace"){
					...
					crt_constraint=readRoomNotAvailableTimes(xmlReader, log);
				}
					...
					crt_constraint=readSubjectPreferredRoom(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintSubjectPreferredRooms"){
					crt_constraint=readSubjectPreferredRooms(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintSubjectSubjectTagPreferredRoom"){
					crt_constraint=readSubjectSubjectTagPreferredRoom(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintSubjectSubjectTagPreferredRooms"){
					crt_constraint=readSubjectSubjectTagPreferredRooms(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintSubjectActivityTagPreferredRoom"){
					crt_constraint=readSubjectActivityTagPreferredRoom(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintSubjectActivityTagPreferredRooms"){
					crt_constraint=readSubjectActivityTagPreferredRooms(xmlReader, log);
				}
				//added 6 apr 2009
				else if(xmlReader.name()=="ConstraintActivityTagPreferredRoom"){
					crt_constraint=readActivityTagPreferredRoom(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintActivityTagPreferredRooms"){
					crt_constraint=readActivityTagPreferredRooms(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintStudentsSetHomeRoom"){
					crt_constraint=readStudentsSetHomeRoom(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintStudentsSetHomeRooms"){
					crt_constraint=readStudentsSetHomeRooms(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintTeacherHomeRoom"){
					crt_constraint=readTeacherHomeRoom(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintTeacherHomeRooms"){
					crt_constraint=readTeacherHomeRooms(xmlReader, log);
				}
					...
					crt_constraint=readTeacherMaxBuildingChangesPerDay(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintTeachersMaxBuildingChangesPerDay"){
					crt_constraint=readTeachersMaxBuildingChangesPerDay(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintTeacherMaxBuildingChangesPerWeek"){
					crt_constraint=readTeacherMaxBuildingChangesPerWeek(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintTeachersMaxBuildingChangesPerWeek"){
					crt_constraint=readTeachersMaxBuildingChangesPerWeek(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintTeacherMinGapsBetweenBuildingChanges"){
					crt_constraint=readTeacherMinGapsBetweenBuildingChanges(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintTeachersMinGapsBetweenBuildingChanges"){
					crt_constraint=readTeachersMinGapsBetweenBuildingChanges(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintStudentsSetMaxBuildingChangesPerDay"){
					crt_constraint=readStudentsSetMaxBuildingChangesPerDay(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintStudentsMaxBuildingChangesPerDay"){
					crt_constraint=readStudentsMaxBuildingChangesPerDay(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintStudentsSetMaxBuildingChangesPerWeek"){
					crt_constraint=readStudentsSetMaxBuildingChangesPerWeek(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintStudentsMaxBuildingChangesPerWeek"){
					crt_constraint=readStudentsMaxBuildingChangesPerWeek(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintStudentsSetMinGapsBetweenBuildingChanges"){
					crt_constraint=readStudentsSetMinGapsBetweenBuildingChanges(xmlReader, log);
				}
				else if(xmlReader.name()=="ConstraintStudentsMinGapsBetweenBuildingChanges"){
					crt_constraint=readStudentsMinGapsBetweenBuildingChanges(xmlReader, log);
				}
////////////////2012-04-29
				else if(xmlReader.name()=="ConstraintActivitiesOccupyMaxDifferentRooms"){
					crt_constraint=readActivitiesOccupyMaxDifferentRooms(xmlReader, log);
				}
////////////////
////////////////2013-09-14
				else if(xmlReader.name()=="ConstraintActivitiesSameRoomIfConsecutive"){
					crt_constraint=readActivitiesSameRoomIfConsecutive(xmlReader, log);
				}
////////////////
				else{
					xmlReader.skipCurrentElement();
...
//corruptConstraintSpace:
				...
				if(xmlReader.name()=="GroupActivitiesInInitialOrder"){
					...
					while(xmlReader.readNextStartElement()){
						...
						if(xmlReader.name()=="Number_of_Activities"){
							QString text=xmlReader.readElementText();
							nActs=text.toInt();
							log.verbose("    Read n_activities="+CustomFETString::number(nActs)+"\n");
						}
						else if(xmlReader.name()=="Activity_Id"){
							QString text=xmlReader.readElementText();
							int id=text.toInt();
							log.verbose("    Activity id="+CustomFETString::number(id)+"\n");
							item->ids.append(id);
						}
						else if(xmlReader.name()=="Active"){
							QString text=xmlReader.readElementText();
			...
			log.minimum("Added "+CustomFETString::number(tgol)+" timetable generation options\n");
		}
		else{
			xmlReader.skipCurrentElement();
			log.numberOfUnrecognizedFields++;
	...
	/*log.minimum("\n");
	log.minimum("Note: if you have overlapping students sets (years or groups), a group or a subgroup may be counted more than once. "
	...
	if(log.numberOfUnrecognizedFields>0){
		log.minimum("\n");
		log.minimum("Warning: There were "+CustomFETString::number(log.numberOfUnrecognizedFields)+" unrecognized fields in the input file\n");
	}
...
TimeConstraint* Rules::readBasicCompulsoryTime(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight"){
			...
			xmlReader.skipCurrentElement();
			log.verbose("    Ignoring old tag - weight - generating automatic 100% weight percentage\n");
...
TimeConstraint* Rules::readTeacherNotAvailable(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight_Percentage"){
			...
			log.verbose("    Read weight percentage="+CustomFETString::number(weightPercentage)+"\n");
		}
		else if(xmlReader.name()=="Active"){
			QString text=xmlReader.readElementText();
			if(text=="false"){
			...
			log.verbose("    Crt. day="+this->daysOfTheWeek[d]+"\n");
		}
			...
			log.verbose("    Start hour="+this->hoursOfTheDay[h1]+"\n");
		}
			...
			log.verbose("    End hour="+this->hoursOfTheDay[h2]+"\n");
		}
		else if(xmlReader.name()=="Teacher_Name"){
			QString text=xmlReader.readElementText();
			teacher=text;
			log.verbose("    Read teacher name="+teacher+"\n");
		}
		else{
			xmlReader.skipCurrentElement();
			log.numberOfUnrecognizedFields++;
...
TimeConstraint* Rules::readTeacherNotAvailableTimes(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight_Percentage"){
			...
			log.verbose("    Read number of not available times="+CustomFETString::number(nNotAvailableSlots)+"\n");
		}

		else if(xmlReader.name()=="Not_Available_Time"){
			log.verbose("    Read: not available time\n");
			...
			while(xmlReader.readNextStartElement()){
				...
				if(xmlReader.name()=="Day"){
					...
					assert(d<this->nDaysPerWeek);
					log.verbose("    Day="+this->daysOfTheWeek[d]+"("+CustomFETString::number(i)+")"+"\n");
				}
				else if(xmlReader.name()=="Hour"){
					QString text=xmlReader.readElementText();
					...
					log.verbose("    Hour="+this->hoursOfTheDay[h]+"\n");
				}
				else{
					xmlReader.skipCurrentElement();
					log.numberOfUnrecognizedFields++;
			...
			QString text=xmlReader.readElementText();
			cn->teacher=text;
			log.verbose("    Read teacher name="+cn->teacher+"\n");
		}
		else{
			xmlReader.skipCurrentElement();
			log.numberOfUnrecognizedFields++;
...
TimeConstraint* Rules::readTeacherMaxDaysPerWeek(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight"){
			//cn->weight=customFETStrToDouble(text);
			xmlReader.skipCurrentElement();
			log.verbose("    Ignoring old tag - weight - generating 100% weight percentage\n");
			cn->weightPercentage=100;
		}
		else if(xmlReader.name()=="Weight_Percentage"){
			QString text=xmlReader.readElementText();
			...
			log.verbose("    Max. days per week="+CustomFETString::number(cn->maxDaysPerWeek)+"\n");
		}
		else{
			xmlReader.skipCurrentElement();
			log.numberOfUnrecognizedFields++;
...
TimeConstraint* Rules::readTeachersMaxDaysPerWeek(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readTeacherMinDaysPerWeek(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight_Percentage"){
			...
			log.verbose("    Min. days per week="+CustomFETString::number(cn->minDaysPerWeek)+"\n");
		}
		else{
			xmlReader.skipCurrentElement();
			log.numberOfUnrecognizedFields++;
...
TimeConstraint* Rules::readTeachersMinDaysPerWeek(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readTeacherIntervalMaxDaysPerWeek(QWidget* parent, QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight"){
			...
			assert(h1>=0 && h1 < this->nHoursPerDay);
			log.verbose("    Interval start hour="+this->hoursOfTheDay[h1]+"\n");
			cn->startHour=h1;
		}
		else if(xmlReader.name()=="Interval_End_Hour"){
			QString text=xmlReader.readElementText();
			if(text==""){
				log.verbose("    Interval end hour void, meaning end of day\n");
				cn->endHour=this->nHoursPerDay;
			}
			else{
				for(h2=0; h2 < this->nHoursPerDay; h2++)
				...
				assert(h2>=0 && h2 < this->nHoursPerDay);
				log.verbose("    Interval end hour="+this->hoursOfTheDay[h2]+"\n");
				cn->endHour=h2;
			}
		}
		else{
...
TimeConstraint* Rules::readStudentsSetMaxDaysPerWeek(QWidget* parent, QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight_Percentage"){
			...
			QString text=xmlReader.readElementText();
			cn->students=text;
			log.verbose("    Read students set name="+cn->students+"\n");
		}
		else if(xmlReader.name()=="Max_Days_Per_Week"){
			QString text=xmlReader.readElementText();
			cn->maxDaysPerWeek=text.toInt();
...
TimeConstraint* Rules::readStudentsSetNotAvailable(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight_Percentage"){
			...
			QString text=xmlReader.readElementText();
			students=text;
			log.verbose("    Read students name="+students+"\n");
		}
		else{
			xmlReader.skipCurrentElement();
			log.numberOfUnrecognizedFields++;
...
TimeConstraint* Rules::readStudentsSetNotAvailableTimes(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight_Percentage"){
			...
			QString text=xmlReader.readElementText();
			cn->students=text;
			log.verbose("    Read students name="+cn->students+"\n");
		}
		else{
			xmlReader.skipCurrentElement();
			log.numberOfUnrecognizedFields++;
...
TimeConstraint* Rules::readMinNDaysBetweenActivities(QWidget* parent, QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight"){
			//cn->weight=customFETStrToDouble(text);
			xmlReader.skipCurrentElement();
			log.verbose("    Ignoring old tag - weight - generating weightPercentage=95%\n");
			cn->weightPercentage=95;
		}
		else if(xmlReader.name()=="Weight_Percentage"){
			QString text=xmlReader.readElementText();
			cn->weightPercentage=customFETStrToDouble(text);
			log.verbose("    Adding weightPercentage="+CustomFETString::number(cn->weightPercentage)+"\n");
		}
		else if(xmlReader.name()=="Active"){
			QString text=xmlReader.readElementText();
			if(text=="false"){
			...
			if(text=="yes" || text=="true" || text=="1"){
				...
				foundCISD=true;
				log.verbose("    Current constraint has consecutive if same day=true\n");
			}
			else{
				if(!(text=="no" || text=="false" || text=="0")){
					RulesReconcilableMessage::warning(parent, tr("FET warning"),
				...
				foundCISD=true;
				log.verbose("    Current constraint has consecutive if same day=false\n");
			}
		}
		else if(xmlReader.name()=="Compulsory"){
			QString text=xmlReader.readElementText();
			...
			log.verbose("    Read n_activities="+CustomFETString::number(cn->n_activities)+"\n");
		}
		else if(xmlReader.name()=="Activity_Id"){
			QString text=xmlReader.readElementText();
			...
			log.verbose("    Read activity id="+CustomFETString::number(cn->activitiesId[n_act])+"\n");
			n_act++;
		}
		else if(xmlReader.name()=="MinDays"){
			QString text=xmlReader.readElementText();
			cn->minDays=text.toInt();
			log.verbose("    Read MinDays="+CustomFETString::number(cn->minDays)+"\n");
		}
		else{
			xmlReader.skipCurrentElement();
			log.numberOfUnrecognizedFields++;
	...
	if(!foundCISD){
		log.verbose("    Could not find consecutive if same day information - making it true\n");
		cn->consecutiveIfSameDay=true;
	}
...
TimeConstraint* Rules::readMaxDaysBetweenActivities(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight_Percentage"){
			...
			log.verbose("    Read MaxDays="+CustomFETString::number(cn->maxDays)+"\n");
		}
		else{
			xmlReader.skipCurrentElement();
			log.numberOfUnrecognizedFields++;
...
TimeConstraint* Rules::readMinGapsBetweenActivities(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight_Percentage"){
			...
			log.verbose("    Read MinGaps="+CustomFETString::number(cn->minGaps)+"\n");
		}
		else{
			xmlReader.skipCurrentElement();
			log.numberOfUnrecognizedFields++;
...
TimeConstraint* Rules::readActivitiesNotOverlapping(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readActivitiesSameStartingTime(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readActivitiesSameStartingHour(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readActivitiesSameStartingDay(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readTeachersMaxHoursDaily(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight"){
			...
			log.verbose("    Read maxHoursDaily="+CustomFETString::number(cn->maxHoursDaily)+"\n");
		}
		else{
			xmlReader.skipCurrentElement();
			log.numberOfUnrecognizedFields++;
...
TimeConstraint* Rules::readTeacherMaxHoursDaily(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readTeachersMaxHoursContinuously(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight"){
			...
			log.verbose("    Read maxHoursContinuously="+CustomFETString::number(cn->maxHoursContinuously)+"\n");
		}
		else{
			xmlReader.skipCurrentElement();
			log.numberOfUnrecognizedFields++;
...
TimeConstraint* Rules::readTeacherMaxHoursContinuously(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readTeacherActivityTagMaxHoursContinuously(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readTeachersActivityTagMaxHoursContinuously(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readTeacherActivityTagMaxHoursDaily(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readTeachersActivityTagMaxHoursDaily(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readTeachersMinHoursDaily(QWidget* parent, QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight"){
			...
			log.verbose("    Read minHoursDaily="+CustomFETString::number(cn->minHoursDaily)+"\n");
		}
		else if(xmlReader.name()=="Allow_Empty_Days"){
			QString text=xmlReader.readElementText();
			if(text=="true" || text=="1" || text=="yes"){
				log.verbose("    Read allow empty days=true\n");
				cn->allowEmptyDays=true;
			}
			else{
				if(!(text=="no" || text=="false" || text=="0")){
				...
				log.verbose("    Read allow empty days=false\n");
...
TimeConstraint* Rules::readStudentsMaxHoursDaily(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readStudentsSetMaxHoursDaily(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readStudentsMaxHoursContinuously(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readStudentsSetMaxHoursContinuously(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readStudentsSetActivityTagMaxHoursContinuously(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readStudentsActivityTagMaxHoursContinuously(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readStudentsSetActivityTagMaxHoursDaily(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readStudentsActivityTagMaxHoursDaily(QXmlStreamReader& xmlReader, XmlLog &log){
...
bool& reportUnspecifiedPermanentlyLockedTime, bool& reportUnspecifiedDayOrHourPreferredStartingTime){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight"){
			...
			QString text=xmlReader.readElementText();
			if(text=="true" || text=="1" || text=="yes"){
				log.verbose("    Permanently locked\n");
				...
				//assert(text=="false" || text=="0" || text=="no");
				log.verbose("    Not permanently locked\n");
			...
			log.verbose("    Read activity id="+CustomFETString::number(cn->activityId)+"\n");
		}
			...
			log.verbose("    Preferred day="+this->daysOfTheWeek[cn->day]+"\n");
		}
			...
			log.verbose("    Preferred hour="+this->hoursOfTheDay[cn->hour]+"\n");
		}
		else{
			xmlReader.skipCurrentElement();
			log.numberOfUnrecognizedFields++;
...
TimeConstraint* Rules::readActivityEndsStudentsDay(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readActivitiesEndStudentsDay(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight_Percentage"){
			...
			log.verbose("    Read students name="+cn->studentsName+"\n");
		}
		else if(xmlReader.name()=="Subject_Name"){
			QString text=xmlReader.readElementText();
			cn->subjectName=text;
			log.verbose("    Read subject name="+cn->subjectName+"\n");
		}
...
TimeConstraint* Rules::read2ActivitiesConsecutive(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::read2ActivitiesGrouped(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::read3ActivitiesGrouped(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight_Percentage"){
			...
			log.verbose("    Read third activity id="+CustomFETString::number(cn->thirdActivityId)+"\n");
		}
		else{
			xmlReader.skipCurrentElement();
			log.numberOfUnrecognizedFields++;
...
TimeConstraint* Rules::read2ActivitiesOrdered(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readTwoActivitiesConsecutive(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readTwoActivitiesGrouped(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readThreeActivitiesGrouped(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readTwoActivitiesOrdered(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readActivityPreferredTimes(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight"){
			...
			log.verbose("    Read number of preferred times="+CustomFETString::number(cn->nPreferredStartingTimes_L)+"\n");
		}
		else if(xmlReader.name()=="Preferred_Time"){
			log.verbose("    Read: preferred time\n");
			...
			while(xmlReader.readNextStartElement()){
				...
				if(xmlReader.name()=="Preferred_Day"){
					...
					log.verbose("    Preferred day="+this->daysOfTheWeek[cn->days_L[i]]+"("+CustomFETString::number(i)+")"+"\n");
				}
				else if(xmlReader.name()=="Preferred_Hour"){
					QString text=xmlReader.readElementText();
					cn->hours_L.append(0);
					...
					log.verbose("    Preferred hour="+this->hoursOfTheDay[cn->hours_L[i]]+"\n");
				}
				else{
					xmlReader.skipCurrentElement();
					log.numberOfUnrecognizedFields++;
...
TimeConstraint* Rules::readActivityPreferredTimeSlots(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight"){
			...
			log.verbose("    Read activity id="+CustomFETString::number(cn->p_activityId)+"\n");
		}
		else if(xmlReader.name()=="Number_of_Preferred_Time_Slots"){
			QString text=xmlReader.readElementText();
			cn->p_nPreferredTimeSlots_L=text.toInt();
			log.verbose("    Read number of preferred times="+CustomFETString::number(cn->p_nPreferredTimeSlots_L)+"\n");
		}
		else if(xmlReader.name()=="Preferred_Time_Slot"){
			log.verbose("    Read: preferred time slot\n");
			...
			while(xmlReader.readNextStartElement()){
				...
				if(xmlReader.name()=="Preferred_Day"){
					...
					log.verbose("    Preferred day="+this->daysOfTheWeek[cn->p_days_L[i]]+"("+CustomFETString::number(i)+")"+"\n");
				}
				else if(xmlReader.name()=="Preferred_Hour"){
					QString text=xmlReader.readElementText();
					...
					log.verbose("    Preferred hour="+this->hoursOfTheDay[cn->p_hours_L[i]]+"\n");
				}
				else{
					xmlReader.skipCurrentElement();
					log.numberOfUnrecognizedFields++;
...
TimeConstraint* Rules::readActivityPreferredStartingTimes(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight"){
			...
			QString text=xmlReader.readElementText();
			cn->nPreferredStartingTimes_L=text.toInt();
			log.verbose("    Read number of preferred starting times="+CustomFETString::number(cn->nPreferredStartingTimes_L)+"\n");
		}
		else if(xmlReader.name()=="Preferred_Starting_Time"){
			log.verbose("    Read: preferred starting time\n");
			...
			while(xmlReader.readNextStartElement()){
				...
				if(xmlReader.name()=="Preferred_Starting_Day"){
					...
					assert(cn->days_L[i]<this->nDaysPerWeek);
					log.verbose("    Preferred starting day="+this->daysOfTheWeek[cn->days_L[i]]+"("+CustomFETString::number(i)+")"+"\n");
				}
					...
					assert(cn->hours_L[i]>=0 && cn->hours_L[i] < this->nHoursPerDay);
					log.verbose("    Preferred starting hour="+this->hoursOfTheDay[cn->hours_L[i]]+"\n");
				}
				else{
					xmlReader.skipCurrentElement();
					log.numberOfUnrecognizedFields++;
...
TimeConstraint* Rules::readBreak(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readBreakTimes(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight_Percentage"){
			...
			QString text=xmlReader.readElementText();
			nNotAvailableSlots=text.toInt();
			log.verbose("    Read number of break times="+CustomFETString::number(nNotAvailableSlots)+"\n");
		}
...
TimeConstraint* Rules::readTeachersNoGaps(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readTeachersMaxGapsPerWeek(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight"){
			...
			log.verbose("    Adding max gaps="+CustomFETString::number(cn->maxGaps)+"\n");
		}
		else if(xmlReader.name()=="Compulsory"){
			QString text=xmlReader.readElementText();
			if(text=="yes"){
...
TimeConstraint* Rules::readTeacherMaxGapsPerWeek(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readTeachersMaxGapsPerDay(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readTeacherMaxGapsPerDay(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readStudentsNoGaps(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readStudentsSetNoGaps(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readStudentsMaxGapsPerWeek(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readStudentsSetMaxGapsPerWeek(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readStudentsMaxGapsPerDay(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readStudentsSetMaxGapsPerDay(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readStudentsEarly(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readStudentsEarlyMaxBeginningsAtSecondHour(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight"){
			...
			log.verbose("    Adding max beginnings at second hour="+CustomFETString::number(cn->maxBeginningsAtSecondHour)+"\n");
		}
		else if(xmlReader.name()=="Compulsory"){
			QString text=xmlReader.readElementText();
			if(text=="yes"){
...
TimeConstraint* Rules::readStudentsSetEarly(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readStudentsSetEarlyMaxBeginningsAtSecondHour(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readActivitiesPreferredTimes(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readActivitiesPreferredTimeSlots(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight"){
			...
			log.verbose("    Read teacher name="+cn->p_teacherName+"\n");
		}
		else if(xmlReader.name()=="Students_Name"){
			QString text=xmlReader.readElementText();
			cn->p_studentsName=text;
			log.verbose("    Read students name="+cn->p_studentsName+"\n");
		}
		else if(xmlReader.name()=="Subject_Name"){
			QString text=xmlReader.readElementText();
			cn->p_subjectName=text;
			log.verbose("    Read subject name="+cn->p_subjectName+"\n");
		}
		else if(xmlReader.name()=="Subject_Tag_Name"){
			QString text=xmlReader.readElementText();
...
TimeConstraint* Rules::readActivitiesPreferredStartingTimes(QXmlStreamReader& xmlReader, XmlLog &log){
...
TimeConstraint* Rules::readSubactivitiesPreferredTimeSlots(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight"){
			...
			log.verbose("    Adding component number="+CustomFETString::number(cn->componentNumber)+"\n");
		}
		else if(xmlReader.name()=="Compulsory"){
			QString text=xmlReader.readElementText();
			if(text=="yes"){
...
//2011-09-25
TimeConstraint* Rules::readActivitiesOccupyMaxTimeSlotsFromSelection(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight_Percentage"){
			...
			QString text=xmlReader.readElementText();
			ac=text.toInt();
			log.verbose("    Read number of activities="+CustomFETString::number(ac)+"\n");
		}
		else if(xmlReader.name()=="Activity_Id"){
			QString text=xmlReader.readElementText();
			cn->activitiesIds.append(text.toInt());
			log.verbose("    Read activity id="+CustomFETString::number(cn->activitiesIds[cn->activitiesIds.count()-1])+"\n");
		}
		else if(xmlReader.name()=="Number_of_Selected_Time_Slots"){
			QString text=xmlReader.readElementText();
			tsc=text.toInt();
			log.verbose("    Read number of selected time slots="+CustomFETString::number(tsc)+"\n");
		}
		else if(xmlReader.name()=="Selected_Time_Slot"){
			log.verbose("    Read: selected time slot\n");
			...
			while(xmlReader.readNextStartElement()){
				...
				if(xmlReader.name()=="Selected_Day"){
					...
					log.verbose("    Selected day="+this->daysOfTheWeek[cn->selectedDays[i]]+"("+CustomFETString::number(i)+")"+"\n");
				}
					...
					log.verbose("    Selected hour="+this->hoursOfTheDay[cn->selectedHours[i]]+"\n");
				}
				else{
					xmlReader.skipCurrentElement();
					log.numberOfUnrecognizedFields++;
			...
			log.verbose("    Read max number of occupied time slots="+CustomFETString::number(cn->maxOccupiedTimeSlots)+"\n");
		}
		else{
			xmlReader.skipCurrentElement();
			log.numberOfUnrecognizedFields++;
...
//2011-09-30
TimeConstraint* Rules::readActivitiesMaxSimultaneousInSelectedTimeSlots(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight_Percentage"){
			...
			log.verbose("    Read max number of simultaneous activities="+CustomFETString::number(cn->maxSimultaneous)+"\n");
		}
		else{
			xmlReader.skipCurrentElement();
			log.numberOfUnrecognizedFields++;
...
/////2017-02-07
TimeConstraint* Rules::readTeacherMaxSpanPerDay(QXmlStreamReader& xmlReader, XmlLog& log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight_Percentage"){
			...
			log.verbose("    Adding max span per day="+CustomFETString::number(cn->maxSpanPerDay)+"\n");
		}
		else{
			xmlReader.skipCurrentElement();
			log.numberOfUnrecognizedFields++;
...
TimeConstraint* Rules::readTeachersMaxSpanPerDay(QXmlStreamReader& xmlReader, XmlLog& log){
...
TimeConstraint* Rules::readStudentsSetMaxSpanPerDay(QXmlStreamReader& xmlReader, XmlLog& log){
...
TimeConstraint* Rules::readStudentsMaxSpanPerDay(QXmlStreamReader& xmlReader, XmlLog& log){
...
TimeConstraint* Rules::readTeacherMinRestingHours(QXmlStreamReader& xmlReader, XmlLog& log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight_Percentage"){
			...
			log.verbose("    Adding min resting hours="+CustomFETString::number(cn->minRestingHours)+"\n");
		}
		else{
			xmlReader.skipCurrentElement();
			log.numberOfUnrecognizedFields++;
...
TimeConstraint* Rules::readTeachersMinRestingHours(QXmlStreamReader& xmlReader, XmlLog& log){
...
TimeConstraint* Rules::readStudentsSetMinRestingHours(QXmlStreamReader& xmlReader, XmlLog& log){
...
TimeConstraint* Rules::readStudentsMinRestingHours(QXmlStreamReader& xmlReader, XmlLog& log){
...
TimeConstraint *Rules::readStudentsSetMinContinuousGapInInterval(QXmlStreamReader &xmlReader, XmlLog &log)
{
	assert(xmlReader.isStartElement() && xmlReader.name()=="ConstraintStudentsSetMinContinuousGapInInterval");
	ConstraintStudentsSetMinContinuousGapInInterval* cn = new ConstraintStudentsSetMinContinuousGapInInterval();
	while(xmlReader.readNextStartElement()){
		log.verbose("    Found "+xmlReader.name().toString()+" tag\n");
		if(xmlReader.name()=="Weight_Percentage"){
			QString text=xmlReader.readElementText();
...
TimeConstraint *Rules::readStudentsMinContinuousGapInInterval(QXmlStreamReader &xmlReader, XmlLog &log)
{
	assert(xmlReader.isStartElement() && xmlReader.name()=="ConstraintStudentsMinContinuousGapInInterval");
	ConstraintStudentsMinContinuousGapInInterval* cn = new ConstraintStudentsMinContinuousGapInInterval();
	while(xmlReader.readNextStartElement()){
		log.verbose("    Found "+xmlReader.name().toString()+" tag\n");
		if(xmlReader.name()=="Weight_Percentage"){
			QString text=xmlReader.readElementText();
...
SpaceConstraint* Rules::readBasicCompulsorySpace(QXmlStreamReader& xmlReader, XmlLog &log){
...
SpaceConstraint* Rules::readRoomNotAvailable(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight_Percentage"){
			...
			QString text=xmlReader.readElementText();
			room=text;
			log.verbose("    Read room name="+room+"\n");
		}
		else{
			xmlReader.skipCurrentElement();
			log.numberOfUnrecognizedFields++;
...
SpaceConstraint* Rules::readRoomNotAvailableTimes(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight_Percentage"){
			...
			QString text=xmlReader.readElementText();
			cn->room=text;
			log.verbose("    Read room name="+cn->room+"\n");
		}
		else{
			xmlReader.skipCurrentElement();
			log.numberOfUnrecognizedFields++;
...
SpaceConstraint* Rules::readActivityPreferredRooms(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		/*if(xmlReader.name()=="Weight"){
			...
			log.verbose("    Read number of preferred rooms: "+CustomFETString::number(_n_preferred_rooms)+"\n");
...
SpaceConstraint* Rules::readSubjectPreferredRoom(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight"){
			...
			QString text=xmlReader.readElementText();
			cn->subjectName=text;
			log.verbose("    Read subject="+cn->subjectName+"\n");
		}
		else if(xmlReader.name()=="Room"){
			QString text=xmlReader.readElementText();
			cn->roomName=text;
...
SpaceConstraint* Rules::readSubjectPreferredRooms(QXmlStreamReader& xmlReader, XmlLog &log){
...
SpaceConstraint* Rules::readSubjectSubjectTagPreferredRoom(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight"){
			...
			QString text=xmlReader.readElementText();
			cn->activityTagName=text;
			log.verbose("    Read activity tag="+cn->activityTagName+"\n");
		}
		else if(xmlReader.name()=="Room"){
			QString text=xmlReader.readElementText();
			cn->roomName=text;
...
SpaceConstraint* Rules::readSubjectSubjectTagPreferredRooms(QXmlStreamReader& xmlReader, XmlLog &log){
...
SpaceConstraint* Rules::readSubjectActivityTagPreferredRoom(QXmlStreamReader& xmlReader, XmlLog &log){
...
SpaceConstraint* Rules::readSubjectActivityTagPreferredRooms(QXmlStreamReader& xmlReader, XmlLog &log){
...
SpaceConstraint* Rules::readActivityTagPreferredRoom(QXmlStreamReader& xmlReader, XmlLog &log){
	//added 6 apr 2009
...
SpaceConstraint* Rules::readActivityTagPreferredRooms(QXmlStreamReader& xmlReader, XmlLog &log){
...
SpaceConstraint* Rules::readStudentsSetHomeRoom(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight_Percentage"){
			...
			QString text=xmlReader.readElementText();
			cn->studentsName=text;
			log.verbose("    Read students="+cn->studentsName+"\n");
		}
		else if(xmlReader.name()=="Room"){
			QString text=xmlReader.readElementText();
			cn->roomName=text;
...
SpaceConstraint* Rules::readStudentsSetHomeRooms(QXmlStreamReader& xmlReader, XmlLog &log){
...
SpaceConstraint* Rules::readTeacherHomeRoom(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight_Percentage"){
			...
			QString text=xmlReader.readElementText();
			cn->teacherName=text;
			log.verbose("    Read teacher="+cn->teacherName+"\n");
		}
		else if(xmlReader.name()=="Room"){
			QString text=xmlReader.readElementText();
			cn->roomName=text;
...
SpaceConstraint* Rules::readTeacherHomeRooms(QXmlStreamReader& xmlReader, XmlLog &log){
...
SpaceConstraint* Rules::readTeacherMaxBuildingChangesPerDay(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight_Percentage"){
			...
			log.verbose("    Max. building changes per day="+CustomFETString::number(cn->maxBuildingChangesPerDay)+"\n");
		}
		else{
			xmlReader.skipCurrentElement();
			log.numberOfUnrecognizedFields++;
...
SpaceConstraint* Rules::readTeachersMaxBuildingChangesPerDay(QXmlStreamReader& xmlReader, XmlLog &log){
...
SpaceConstraint* Rules::readTeacherMaxBuildingChangesPerWeek(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight_Percentage"){
			...
			log.verbose("    Max. building changes per week="+CustomFETString::number(cn->maxBuildingChangesPerWeek)+"\n");
		}
		else{
			xmlReader.skipCurrentElement();
			log.numberOfUnrecognizedFields++;
...
SpaceConstraint* Rules::readTeachersMaxBuildingChangesPerWeek(QXmlStreamReader& xmlReader, XmlLog &log){
...
SpaceConstraint* Rules::readTeacherMinGapsBetweenBuildingChanges(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight_Percentage"){
			...
			log.verbose("    Min gaps between building changes="+CustomFETString::number(cn->minGapsBetweenBuildingChanges)+"\n");
		}
		else{
			xmlReader.skipCurrentElement();
			log.numberOfUnrecognizedFields++;
...
SpaceConstraint* Rules::readTeachersMinGapsBetweenBuildingChanges(QXmlStreamReader& xmlReader, XmlLog &log){
...
SpaceConstraint* Rules::readStudentsSetMaxBuildingChangesPerDay(QXmlStreamReader& xmlReader, XmlLog &log){
...
SpaceConstraint* Rules::readStudentsMaxBuildingChangesPerDay(QXmlStreamReader& xmlReader, XmlLog &log){
...
SpaceConstraint* Rules::readStudentsSetMaxBuildingChangesPerWeek(QXmlStreamReader& xmlReader, XmlLog &log){
...
SpaceConstraint* Rules::readStudentsMaxBuildingChangesPerWeek(QXmlStreamReader& xmlReader, XmlLog &log){
...
SpaceConstraint* Rules::readStudentsSetMinGapsBetweenBuildingChanges(QXmlStreamReader& xmlReader, XmlLog &log){
...
SpaceConstraint* Rules::readStudentsMinGapsBetweenBuildingChanges(QXmlStreamReader& xmlReader, XmlLog &log){
...
//2012-04-29
SpaceConstraint* Rules::readActivitiesOccupyMaxDifferentRooms(QXmlStreamReader& xmlReader, XmlLog &log){
	...
	while(xmlReader.readNextStartElement()){
		...
		if(xmlReader.name()=="Weight_Percentage"){
			...
			log.verbose("    Read max number of different rooms="+CustomFETString::number(cn->maxDifferentRooms)+"\n");
		}
		else{
			xmlReader.skipCurrentElement();
			log.numberOfUnrecognizedFields++;
...
//2013-09-14
SpaceConstraint* Rules::readActivitiesSameRoomIfConsecutive(QXmlStreamReader& xmlReader, XmlLog &log){
