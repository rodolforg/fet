...
void EditableTimetableWidget::contextMenuEvent(QContextMenuEvent* event)
{
	...
	QSet<int> all_activity_ids_but_clicked = placed_activity_ids_but_clicked + tempRemovedActivities;
	foreach (int ai, all_activity_ids_but_clicked) {
...
void EditableTimetableWidget::placeActivity(const QTableWidgetItem* item, int ai)
{
	...
	foreach (int remAi, tempRemovedActivities) {
